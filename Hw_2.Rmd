...
```
This graph compares UT Austin male professor's course evaluations to female professors. There is a peak for the female professors at the higher end of the 3.0 scores, while the peak for male professors is at the lower end of the 4.0 scores, indicating the male professors have higher evaluation scores on average.
...
```
Based on the graph there is no obvious correlation between a professor's physical attractiveness and their course evaluation. This can be backed up by looking at the correlation coefficient, which is 0.1890391, indicating a very weak positive linear correlation between the two variables at hand. This means that the physical attractivness of a professor has a weak positive impact on their course evaluations. 
...
ggplot(bike |>
  ...
  #graphing
  geom_line(aes(x=hr, y=hourly_means)) + labs(title = "Average bike rentals per hour", x = "Time (hours)", y= "Number of rides") + theme_minimal()


```
This graph represents the average number of public bike rentals per hour in any given day. There is a spike around 8am and another larger one at 5pm, indicating a sudden increase in bike rental users. This could be a result of people using it to get to work and back home afterwards, as it correlates with the usual work hours of 9 to 5. 
...
ggplot(bike |>
  ...
  facet_wrap(~workingday, labeller = labeller (workingday = c("1" = "Working Day", "0" = "Non-Work Day"))) +  labs(title = "Average bike rentals per hour based on type of day", x = "Hour", y= "Number of rides") + theme_minimal()

```

The first graph represents the average number of public bike rentals per hour in a non-work day, while the second graphs refers to average bike rentals per hour on a weekday. The first graph shows a curve from around noon to 5pm, while the second graph has more rapid sharp spikes at 9am and 5pm. The bike rentals from first graph are due to people traveling for recreational and errand purposes, which is why are they more spread out and not all at the same time, but the working day graph is due to people traveling to and from work which causes the spikes as everyone needs them at a similar time. 
...
ggplot(bike |>
  ...
  labs(title = "Average Rides taken at 9am based on weather", x = "Weather (scale of 1-3)", y = "Average Number of Rides taken") +
  theme_minimal() 
...
This graph shows the number of average rides taken by public bike rental users at 9am segregated by weather as well as if it is a working day or not. Through this we can conclude that the weather does make an impact on the rides taken at 9am on a non-working day, but it does not impact the number of rides taken at that same time on a working day unless the weather gets really bad (categorized as a 3 or higher). This is due to the fact that people have to use the bikes to get to work at 9am, which a mist will not interfere with however heavy rain or a thunderstorm would cause the amount of people going to work to decrease on that given day.
...
ggplot(artists_19, aes(x = performer, y = ten_week_hit_count)) +
  geom_bar(stat = "identity", fill = "skyblue", color = "navy") + coord_flip() +  labs(x = "Performer", y = "Number of 10 Week Hits", 
       title = "Number of Ten-Week Hits by Performers") +
  theme_minimal()

```

The figure shows the 19 performers that have had over thirty 10 week hits on the Billboard 100. A 10 week hit is when the song stays on the Billboard for 10 or more weeks. The performer with the longest hit was Elton John, with 52 weeks on the Billboard. 
